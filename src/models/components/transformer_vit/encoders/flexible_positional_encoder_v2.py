...
class FlexiblePositionalEncoder(nn.Module):
    ...
    Args:
        ...
        use_differences (bool): Whether to include difference embeddings. Default: True
    ...
    def __init__(self, 
        ...
        self.features = self._configure_features(
            use_sinusoidal, use_conv_mag, use_time_diff,
            use_mag_diff, use_rate, use_band_embedding, use_abs_time_mlp, use_abs_mag_mlp
        )
    ...
    def _configure_features(self,
                          use_sinusoidal: bool, use_conv_mag: bool, use_time_diff: bool,
                          use_mag_diff: bool, use_rate: bool, use_band_embedding: bool,
    ...
    def _create_adaptive_mlp(self, input_size: int, reduced_size: int) -> nn.Sequential:
        ...
        # Final normalization
        layers.append(nn.RMSNorm(self.embedding_size))
    ...
    def compute_time_differences(self, t):
        ...
        batch_size, n_patches, seq_len,_ = t.shape
        t_shifted = torch.cat([torch.zeros(batch_size, n_patches, seq_len, 1, device=t.device), t[:, :, :,:-1]], dim=-1)
        time_diffs = torch.abs(t - t_shifted)
        time_difs = time_diffs.unsqueeze(-1)  # Remove last dimension if present
        return time_diffs
    
    def compute_magnitude_differences(self, x):
        """Compute magnitude differences between consecutive steps."""
        batch_size, n_patches, seq_len,_ = x.shape
        x_shifted = torch.cat([torch.zeros(batch_size, n_patches, seq_len, 1, device=x.device), x[:, :,:, :-1]], dim=-1)
    ...
    def magnitud_embedding(self, x):
        ...
        # x has shape [batch, n_patches, patch_len]
        batch, n_patches, patch_len,_ = x.shape  # Remove last dimension if present
        
        # Reshape to process each patch independently
        x = x.view(batch * n_patches, patch_len,1)  # [batch*n_patches, 1, patch_len]
        x = x.transpose(1, 2)  # [batch*n_patches, patch_len, 1]

        # Apply convolution
        x = self.magnitud_emb_big(x)  # [batch*n_patches, conv_out_size, patch_len]
        x = x.transpose(1, 2)  # [batch*n_patches, patch_len, conv_out_size]
        x = x.view(batch, n_patches, patch_len, -1)
        return x
    ...
    def forward(self, x, t, bands=None):
        ...
        if any(self.features[f] for f in ['time_differences', 'rate_of_change']):
            time_diffs = self.compute_time_differences(t)
        if any(self.features[f] for f in ['mag_differences', 'rate_of_change']):
...
class FlexiblePositionalEncoderHandler(nn.Module):
    ...
    def forward(self, x, t, mask=None, band_info=None, **kwargs):
        ...
        """
        batch, n_patches, patch_len = x.shape



        # collect per‐band outputs
        x_mod = torch.zeros(batch, n_patches, patch_len, self.embedding_size, device=x.device)
        band_info = band_info.max(-1)[0].unsqueeze(-1).expand(-1, -1, patch_len)
        for i in range(1, self.num_bands + 1):
            # Create band mask expanded to match dimensions
            band_mask = (band_info == i)
            x_i = torch.where(band_mask, x, torch.zeros_like(x))
            t_i = torch.where(band_mask, t, torch.zeros_like(t))
            b_i = torch.where(band_mask, band_info, torch.zeros_like(mask))

            mod = self.time_encoders[i-1](x_i, t_i, b_i)  # Note: i-1 since ModuleList is 0-indexed

            band_mask_expanded = band_mask.unsqueeze(-1).expand(-1, -1, -1, self.embedding_size)
            # Use the expanded mask for element-wise selection
            x_mod = torch.where(band_mask_expanded, mod, x_mod)
