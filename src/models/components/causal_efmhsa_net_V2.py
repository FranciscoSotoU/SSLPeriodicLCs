...
class EFATAT(nn.Module):
    ...
    def forward(self, data, time, mask=None, bands=None, metadata=None, features=None, period=None, **kwargs):
        ...
        # Add token and process - concatenate in order: feat, lc, clf_token
        if self.token_mode != 'mean':
            batch_size = x_mod_lc.size(0)
            clf_token = self.token(batch_size)
            x_mod = torch.cat([x_mod_feat, x_mod_lc, clf_token], dim=1)
            m_token = torch.ones(batch_size, 1, 1).to(x_mod_lc.device)
            m_mod = torch.cat([m_mod_feat, m_mod_lc, m_token], dim=1)
            x_mod_token, m_mod_token = x_mod, m_mod
        else:
            x_mod = torch.cat([x_mod_feat, x_mod_lc], dim=1)
            m_mod = torch.cat([m_mod_feat, m_mod_lc], dim=1)
            x_mod_token, m_mod_token = x_mod, m_mod
        ...
        if self.token_mode == "mean":
            ...
            # Token is at the end: feat, lc, clf_token
            token_lc = token_lc[:, -1, :]
