...
from .retnet.RetNetV2 import RetNet
from .transformer.handlers import TimeHandler
from .utils import KSparse



class RetNetLightcurve(nn.Module):
    """RetNet Lightcurve processor for handling time series lightcurve data."""
    
    def __init__(self, **kwargs):
        super(RetNetLightcurve, self).__init__()
        ...
        self.retnet_lc = RetNet(**kwargs)
        self.time_encoder = TimeHandler(**kwargs)
        self.k_sparse = KSparse(**kwargs)
        self.token_mode = kwargs.get("token_mode", 'mean')
        self.init_model()
    ...
    def forward(self, data, time, mask=None, bands=None, **kwargs):
        ...
        # Transform through RetNet
        token_lc = self.retnet_lc(
            X=x_mod,
            attention_mask=m_mod.squeeze(-1),
        )
        ...
        # Find the last valid index for each sequence in the mask
        if self.token_mode == "last_valid":
            valid_mask = m_mod.squeeze(-1)  # shape: (batch_size, seq_len)
            last_valid_idx = valid_mask.sum(dim=1).long() - 1  # shape: (batch_size,)
            batch_indices = torch.arange(token_lc.size(0), device=token_lc.device)
            token_lc = token_lc[batch_indices, last_valid_idx, :]  # Gather last valid token for each sequence
        elif self.token_mode == "mean":
            valid_mask = m_mod.squeeze(-1)
            token_lc = token_lc * valid_mask.unsqueeze(-1)  # Zero out masked elements
            token_lc = token_lc.sum(dim=1) / valid_mask.sum(dim=1, keepdim=True)  
