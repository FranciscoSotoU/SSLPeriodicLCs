...
class SimpleRetention(nn.Module):
    def __init__(
            self, hidden_size, gamma, head_size=None, double_v_dim=False, scale_time=180
        ):
        """
        ...
        self.xpos = XPOS(head_size, scale_base=scale_time)
...
class MultiScaleRetention(nn.Module):
    def __init__(self, hidden_size, heads, double_v_dim=False, scale_time=180):
        """
        ...
        self.gammas = (
            1 - torch.exp(torch.linspace(math.log(10/scale_time), math.log(1/scale_time), heads))
        ).detach().cpu().tolist()
        ...
        self.retentions = nn.ModuleList([
            SimpleRetention(
                self.hidden_size, gamma, self.head_size, double_v_dim, scale_time
...
class RetNet(nn.Module):
    def __init__(
            self, embedding_size=272, num_encoders=2, expansion_coef=2, num_heads=16, norm_type="layernorm", double_v_dim=False, scale_time=180, **kwargs
        ):
        ...
        self.retentions = nn.ModuleList([
            MultiScaleRetention(hidden_dim, self.heads, double_v_dim, scale_time)
    ...
    def forward_recurrent(self, x_n, s_n_1s, n):
        """
        X: (batch_size, sequence_length, hidden_size)
        s_n_1s: list of lists of tensors of shape (
            ]batch_size, hidden_size // heads, hidden_size // heads)
    ...
    def forward_chunkwise(self, x_i, r_i_1s, i):
        """
        X: (batch_size, sequence_length, hidden_size)
        r_i_1s: list of lists of tensors of shape (
            batch_size, hidden_size // heads, hidden_size // heads)
        ...
        for j in range(self.layers):
            ...
            x_i = x_i.to(self.layer_norms_1[j].weight.device)
            r_i_1s[j] = r_i_1s[j].to(x_i.device)
...
class RetNetRepresentation(nn.Module):
    ...
    def __init__(
        ...
        self.output = embedding_size

    def forward(self, x):
        """_summary_
        self.cls_token = nn.Parameter(torch.randn(1, 1, input_size))
        # cls_token = self.cls_token.repeat(len(x), 1, 1)
        # x = torch.cat([cls_token, x], dim=1)
        # representations = self.retnet(x)
