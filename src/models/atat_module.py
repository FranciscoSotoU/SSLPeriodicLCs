...
from src.models.components.utils.middle import Concatenation, ConcatenationRelu, ConcatenationDropOut, ConcatenationKSparse,ConcatenationKSparseTwoTails,AttentionBasedConcatenation,ConcatenationNormalization
...
class ATATLitModule(LightningModule):
    ...
    def __init__(
        ...
        mix_classifier: torch.nn.Module = None,
        feat_classifier: torch.nn.Module = None,
        lc_classifier: torch.nn.Module = None,
        ...
        self.save_hyperparameters(logger=False, ignore=['lc_net', 'feat_net', 'mix_classifier', 'feat_classifier', 'lc_classifier'])
        ...
        # metric objects for calculating and averaging accuracy across batches (excluding confusion matrix)
        ...
        self.train_metrics_lc = metrics.clone(prefix='train/', postfix='_lc')
        self.train_metrics_feat = metrics.clone(prefix='train/', postfix='_feat')
        self.train_metrics_mix = metrics.clone(prefix='train/', postfix='_mix')
        self.val_metrics_mix = metrics.clone(prefix='val/', postfix='_mix')
        self.val_metrics_lc = metrics.clone(prefix='val/', postfix='_lc')
        self.val_metrics_feat = metrics.clone(prefix='val/', postfix='_feat')
        self.test_metrics_mix = metrics.clone(prefix='test/', postfix='_mix')
        self.test_metrics_lc = metrics.clone(prefix='test/', postfix='_lc')
        self.test_metrics_feat = metrics.clone(prefix='test/',postfix='_feat')

        
        # confusion matrix metrics (separate to avoid logging issues)
        ...
        self.val_cm_lc = torchmetrics.ConfusionMatrix(task='multiclass', num_classes=num_classes, normalize='true')
        self.val_cm_feat = torchmetrics.ConfusionMatrix(task='multiclass', num_classes=num_classes, normalize='true')
        self.test_cm_lc = torchmetrics.ConfusionMatrix(task='multiclass', num_classes=num_classes, normalize='true')
        self.test_cm_feat = torchmetrics.ConfusionMatrix(task='multiclass', num_classes=num_classes, normalize='true')
        ...
        # Statistics tracking for lc_net and feat_net outputs
        self.train_lc_mean = MeanMetric()
        self.train_lc_var = MeanMetric()
        self.train_feat_mean = MeanMetric()
        self.train_feat_var = MeanMetric()
        self.val_lc_mean = MeanMetric()
        self.val_lc_var = MeanMetric()
        self.val_feat_mean = MeanMetric()
        self.val_feat_var = MeanMetric()
        ...
        concatenations_dict = {
            ...
            'attention': AttentionBasedConcatenation,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.lc_net is not None:
            ...
            lc_branch_out = self.lc_classifier(lc_out)
    ...
    def on_train_start(self) -> None:
        ...
        self.val_metrics_lc.reset()
        self.val_metrics_feat.reset()
        self.val_metrics_mix.reset()
        # Reset confusion matrices
        self.val_cm.reset()
        self.val_cm_lc.reset()
        self.val_cm_feat.reset()
        self.val_loss_min.reset()
        # Reset statistics metrics
        self.val_lc_mean.reset()
        self.val_lc_var.reset()
        self.val_feat_mean.reset()
        self.val_feat_var.reset()
    ...
    def training_step(
        ...
        y = batch['label'].long()
        lcs_preds, feat_preds, mix_preds, lc_out, feat_out = self.forward(batch)
        loss = 0.0
        
        # Collect predictions for main metrics
        main_preds = None
        
        if self.lc_net is not None and lc_out is not None:
            # Track statistics for lc_net output
            lc_mean = lc_out.mean()
            lc_var = lc_out.var()
            self.train_lc_mean(lc_mean)
            self.train_lc_var(lc_var)
            ...
            self.log("train/lc_mean", self.train_lc_mean, on_step=False, on_epoch=True, prog_bar=False)
            self.log("train/lc_var", self.train_lc_var, on_step=False, on_epoch=True, prog_bar=False)
        ...
        if self.feat_net is not None and feat_out is not None:
            # Track statistics for feat_net output
            feat_mean = feat_out.mean()
            feat_var = feat_out.var()
            self.train_feat_mean(feat_mean)
            self.train_feat_var(feat_var)
            ...
            self.log("train/feat_mean", self.train_feat_mean, on_step=False, on_epoch=True, prog_bar=False)
            self.log("train/feat_var", self.train_feat_var, on_step=False, on_epoch=True, prog_bar=False)
    ...
    def validation_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int) -> None:
        ...
        if self.lc_net is not None and lc_out is not None:
            ...
            lc_mean = lc_out.mean()
            lc_var = lc_out.var()
            self.val_lc_mean(lc_mean)
            self.val_lc_var(lc_var)
            ...
            self.log("val/lc_mean", self.val_lc_mean, on_step=False, on_epoch=True, prog_bar=False)
            self.log("val/lc_var", self.val_lc_var, on_step=False, on_epoch=True, prog_bar=False)
        ...
        if self.feat_net is not None and feat_out is not None:
            ...
            feat_mean = feat_out.mean()
            feat_var = feat_out.var()
            self.val_feat_mean(feat_mean)
            self.val_feat_var(feat_var)
            ...
            self.log("val/feat_mean", self.val_feat_mean, on_step=False, on_epoch=True, prog_bar=False)
            self.log("val/feat_var", self.val_feat_var, on_step=False, on_epoch=True, prog_bar=False)
    ...
    def predict_step(self, batch: Tuple[torch.Tensor, torch.Tensor], batch_idx: int, dataloader_idx: int = 0) -> None:
        ...
        if self.UMAP:
            ...
            lcs, feat, mix, lc_out, feat_out = self.forward(batch)
