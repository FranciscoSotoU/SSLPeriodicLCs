...
class ForcedPhotometryDatasetMM(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.normalize_tab:
            # Normalize tabular data if specified
        ...
        logging.info('Loaded {} objects'.format(len(self.target)))
    ...
    def order_sampling(self, idx_, at_time=None):
        ...
        if self.patch_size > 1:
            ...
            ts_data = self.band_patching(ts_data, self.patch_size)
    ...
    def patch_sequence(self, ts_data, patch_size):
        ...
        patched_bands = patched_bands.max(axis=1)  # Reduce bands to max value per patch
        patched_mask = patched_mask.max(axis=1)
        return {
            'data': patched_data,
            'time': patched_time,
            'mask': patched_mask,
    ...
    def apply_max_time_to_eval(self):
        ...
        for base_snid, indices in snid_groups.items():
            ...
            cut = min_time + self.max_time_to_eval
            for idx in indices:
                self.data[idx] = np.where(self.time[idx] <= cut, self.data[idx], 0)
                self.mask[idx] = np.where(self.time[idx] <= cut, self.mask[idx], 0)
                self.time[idx] = np.where(self.time[idx] <= cut, self.time[idx], 0)
                # Check if there are valid non-zero time values after filtering
                if np.any(self.time[idx] > 0):
